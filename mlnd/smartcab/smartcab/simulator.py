        ...
        if status and status['waypoint'] is not None:  # Continuing the trial
            ...
            if status['violation'] == 0:  # Legal
                if status['waypoint'] == status['action']:  # Followed waypoint
                    print "Agent followed the waypoint {}. (rewarded {:.2f})"\
                        .format(status['action'], status['reward'])
                # the agent is idled.
                elif status['action'] == None:
                    if status['light'] == 'red':  # Stuck at red light
                        print "Agent properly idled at a red light. (rewarded {:.2f})"\
                ...
                if status['violation'] == 1:  # Minor violation
                    ...
                    print "Agent attempted driving {} through a red light with traffic and cause a major accident. (rewarded {:.2f})"\
                        .format(status['action'], status['reward'])

            # Time Remaining
            if self.env.enforce_deadline:
                remain = (status['deadline'] - 1) * 100.0 / (status['t'] + status['deadline'])
                print "{:.0f}% of time remaining to reach destination.".format(remain)
            else:
    ...
    def render(self, trial, testing=False):
        ...
        for agent, state in self.env.agent_states.iteritems():
            ...
            if hasattr(agent, '_sprite') and agent._sprite is not None:
                ...
                self.screen.blit(rotated_sprite, self.pygame.rect.Rect(agent_pos[0] - agent._sprite_size[0] / 2,
        ...
        if status:
            ...
            if self.env.enforce_deadline:
                remain = (status['deadline'] - 1) * 100.0 / (status['t'] + status['deadline'])
                self.screen.blit(self.font.render("{:.0f}% of time remaining to reach destination.".format(remain), True,
                                                  self.colors['black'], self.bg_color), (350, 100))
            else:
                self.screen.blit(self.font.render("Agent not enforced to meet deadline.", True, self.colors['black'],
            ...
            if (state['destination'] != state['location'] and state['deadline'] > 0) or (
                ...
                if self.env.success:
                    self.screen.blit(
                        self.font.render("Previous Trial: Success", True, self.colors['dgreen'], self.bg_color), (10, 50))
                if self.env.success:
                    self.screen.blit(
                        self.font.render("Previous Trial: Failure", True, self.colors['maroon'], self.bg_color), (10, 50))
