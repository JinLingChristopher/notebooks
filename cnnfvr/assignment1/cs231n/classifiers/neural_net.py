...
class TwoLayerNet(object):
  ...
  """
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        correct_loss = -np.log(probs[np.arange(len(probs)), y])
        loss = np.sum(correct_loss) / N + reg * (np.sum(W1 ** 2) + np.sum(W2 ** 2))
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        dW2 = np.dot(a1.T, delta2) / N + 2 * reg * W2
        db2 = np.sum(delta2, axis=0) / N

        a1_prime = a1.copy()
        a1_prime[a1 > 0] = 1
        delta1 = np.dot(delta2, W2.T) * a1_prime
        dW1 = np.dot(X.T, delta1) / N + 2 * reg * W1
